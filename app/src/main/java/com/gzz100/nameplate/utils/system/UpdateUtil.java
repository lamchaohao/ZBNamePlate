package com.gzz100.nameplate.utils.system;
...
import static com.gzz100.nameplate.global.Global.KEY_DOWNLOAD_ID;
...
public class UpdateUtil {
    public static void download(Context context, String url, String title) {
        long downloadId = context.getSharedPreferences(Global.SP_SYSTEM,Context.MODE_PRIVATE).getLong(KEY_DOWNLOAD_ID, -1L);
        if (downloadId != -1L) {
    ...
    private static void start(Context context, String url, String title) {
        ...
        sp.edit().putLong(KEY_DOWNLOAD_ID, id).apply();
